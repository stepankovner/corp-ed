...
- `normalize_citations` — страховка ссылок. Даже с правилом в промпте
  модели иногда ставят в скобки номер из самого документа, а не номер
  выдержки: пункт (`[4.2]`, прогон 24.09: 1 ответ из 20) или строку
  таблицы / пункт перечня (`[38]` при пяти выдержках; замер ссылок 24.09
  на 220 вопросах: 9 таких ссылок у Alice Flash, 22 у lite). Функция
  заменяет такую ссылку на номер выдержки, где этот пункт начинает строку
  или есть строка «№: 38», а если такая выдержка не одна — на текст
  «(п. 4.2)». Ссылки `[1]`…`[k]` не трогает. Фронт не получает битых
  ссылок.
