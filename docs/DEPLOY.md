...
только по имени хоста и резолвит его сам. Тогда ставится
`CONNECTOR_OUTBOUND_VIA_PROXY=true`: запрос уходит по имени, проверка
адреса (только `https`, публичный IP, без учётных данных в URL) остаётся,
а защита от подмены DNS между проверкой и подключением ложится на
политику прокси — она и есть та самая вторая линия. Сертификат прокси,
если он перевыпускает TLS, добавляется в доверенные через
`SSL_CERT_FILE` (httpx читает его из окружения). Без прокси флаг не
включать: он снимает закрепление адреса.
