...
клиентов. Одноразовость `state` OAuth держится на том же Redis, что и
лимиты частоты: без Redis (только разработка) — в памяти процесса,
при нескольких процессах API повтор `state` в другой процесс не будет
замечен. Для Битрикс24 нужен выход к порталу клиента (`*.bitrix24.ru`
...
Для Confluence Server/DC — выход к адресу инсталляции клиента (часто
