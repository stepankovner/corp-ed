...
**Версия:** v1.1 · 25.09.2026 (к 29.09) — добавлено решение по задаче 1:
Alice AI LLM Flash + text-embeddings-v2, размерность 768. v2 — к 06.10,
финал — к 13.10.
...
#7–#12 вливаются после #6 — CI у них падает на той же ожидаемой ошибке.
...
- **Что сделать:**
  ...
  4. **С 25.09 эмбеддер — text-embeddings-v2 с размерностью 768** (BH-16,
     BH-17). Миграцию этого пункта лучше сделать вместе с BH-17 и
     переингестить один раз: сразу новой нарезкой и новым эмбеддером.
     Иначе переингестов два с разницей в два дня: 30.09 и 02.10.
...
- **Вместе с BH-3** — одна миграция и один переингест (BH-3, шаг 4).
...
- **Блокирует:** включение v2-768. **Приоритет / Срок:** P1 · 02.10
  (лучше вместе с BH-3 к 30.09 — один переингест).
- **Что сделать:**
  ```sql
