...
**Версия:** v2 · 25.09.2026 (планировалась к 06.10) — все решения по
золотому набору в одном месте; история пунктов BH-1…BH-26 с контрактами
и ловушками — в приложении. Финал (после holdout) — к 13.10.
**Основа:** ТЗ ML-части (23.09), `docs/ml-report.md` (замеры),
`docs/ml-plan.md` (план и расхождения с ТЗ), `docs/ml-summary.md`,
`docs/ml-code-guide.md`.

ML пишет только чистые функции (без БД, сети, файлов), промпты и eval.
Всё, что ходит в БД и во внешние API, — ваше. ML-код в бэкенд-файлы не
лезет; пожелания к бэкенду — только пунктами BH-N здесь.

## 1. Что стоит в продукте и почему (решения на 25.09)

| Параметр | Значение | Где у вас | Основание (`docs/ml-report.md`) |
|---|---|---|---|
| Модель ответа | Alice AI LLM Flash, `/latest`, OpenAI-совместимый API | `LLM_PROVIDER=yandex-openai`, `LLM_MODEL=aliceai-llm-flash` | задача 1: F1 отказа 0,98 против 0,94 у Lite, 0,19 ₽ против 0,37 ₽ за ответ, p95 2 с против 4 с |
| Эмбеддинги | `text-embeddings-v2`, `dim=768`, пары doc / query | `EMBEDDING_MODEL`, `EMBEDDING_DIM=768` | MRR 0,63 против 0,39 у text-search (p = 0,0001) |
| Нарезка | 400/50 токенов по секциям Markdown; крошки «Документ > Раздел» **с названием документа** в `embed_text` | `RAG_CHUNK_TOKENS=400`, `RAG_OVERLAP_TOKENS=50` | золотой dev: без названия −0,22 MRR (p = 0,002), без крошек −0,19 (p = 0,024); 600/50 −0,17 (p = 0,019) |
| Выдержек в ответ | 5, бюджет 3 000 токенов | `RAG_FAQ_LIMIT=5`, `RAG_CONTEXT_MAX_TOKENS=3000` | k = 8: +0,13 балла, но p = 0,25 и +46 % токенов — пересмотр по живым логам |
| Порог отказа | 0,51 (косинусное расстояние лучшего чанка) | `RAG_FAQ_MAX_DISTANCE=0.51` | 0,59 даёт те же ответы на dev; финал — holdout к 12.10 |
| Температура | 0 | `RAG_FAQ_TEMPERATURE=0` | при 0,3 ответ на тот же вопрос переключался «ответил ↔ отказал» |
| Промпт | `faq-v2.4` (`prompts/faq.py`) | `PROMPT_VERSION` в `qa_log` | ссылки `[n]`, `normalize_citations`, общий ответ с пометкой |
| Ответа в документах нет (Р1) | общий ответ со строгой пометкой; строгий отказ — настройка компании | `tenants.not_found_mode` (general \| strict), поле `origin` | решение Артёма 25.09; пометка 96/96; вне корпуса 5/5 верных отказов на dev |
| Поиск | только вектор | `RAG_RETRIEVER=vector` | гибрид на dev — шум (5 вопросов лучше, 8 хуже); помогает лишь на именах («ArendaGo») |
| Словарь сокращений (M5) | таблица есть, по умолчанию пуста | `/api/v1/glossary` | заполняет админ компании; эффект мерить на корпусе клиента |
| Small-to-big (M2) | **не встраивать** до MVP | — | ответы шум (3 лучше / 2 хуже), контекст +58 %; контракт BH-13 — в приложении |
| Multi-query (M6) | **не встраивать** | — | на dev хуже базы при любом весе |
| Отчёт о пробелах | `classify_miss` → кластеры → gaps-v1 | `cli gaps`, `GET /gaps` | синтетика: precision 0,88; пороги `GAPS_STRONG_FULLTEXT` / `GAPS_EMPTY_FULLTEXT` — по живым логам |

**Менять ничего не нужно:** всё это у вас уже стоит (PR #15).

## 2. Статус пунктов BH

| Пункты | Статус |
|---|---|
| BH-1…BH-12, BH-14…BH-24 | ✅ встроены (`claude/gallant-pascal-xtb35t`, PR #15); сверка ML 25.09 с замерами — расхождений нет |
| BH-25 `content_filter` → отказ, BH-26 версия модели в `qa_log` | ✅ в PR #15 (`llm_model_version`) |
| BH-13 M2 small-to-big | ⏸ после MVP, по замеру на золотом dev |
| BH-27 запасная модель при `content_filter` | 🆕 26.09, P3 — решение продукта; описание в приложении |

## 3. Что ML попросит дальше (не срочно)

1. **Стенд на живом API** — «официальные числа» после деплоя: тестовая
   компания, 6 файлов демо-корпуса загружены через `POST /materials`
   (те же файлы, что у ML в `sandbox/raw`), учётка ADMIN — ей доступны
   `/faq/search` и `diagnostics`. Стенд (`eval.run_eval`) читает
   `CORP_ED_BASE_URL`, `CORP_ED_COMPANY`, `CORP_ED_EMAIL`,
   `CORP_ED_PASSWORD` или `CORP_ED_TOKEN` из окружения — секреты в чат
   не нужны. `RAG_*` — как в таблице выше, иначе числа несравнимы.
2. **Живые логи `qa_log`** через несколько недель работы: пороги
   `GAPS_*` (RISKS №16) и пересмотр k = 5 → 8, 400/50 → 600/50 — на dev
   они выглядели чуть лучше, но в пределах шума.
3. **Holdout к 12.10** — если порог изменится, это одно число
   `RAG_FAQ_MAX_DISTANCE`.

## 4. Как забирать обновления ML

Ветки `ml/*` — только новые коммиты и слияния, историю не переписываем.
Вершина стека на 25.09 — `ml/handoff-v2`: влить её — значит получить
всё. PR по порядку (база каждого — предыдущий): #3 `ml/preprocess-split`
→ #4 `ml/retrieval-utils` → #5 `ml/eval` → #6 `ml/prompt-v2` → #7 `ml/prep`
→ #8 `ml/flash-v2` → #9 `ml/golden-tools` → #10 `ml/gaps` → #11
`ml/judge-kappa` → #12 `ml/alice-compare` → #13 `ml/r1-general` → #14
`ml/docs` → #16 `ml/m2-sections` → #17 `ml/golden-runs` → #18
`ml/multi-query` → #19 `ml/handoff-v2`. Если первым вливается ваш #15 в
`main`, GitHub закроет #3–#14 как влитые сам. CI на ML-ветках красный
только из-за ожидаемой ошибки mypy (BH-1 живёт у вас); в `main` после
#15 её нет.

## 5. Открытые вопросы к продукту (не к бэкенду)

- Досье v3.2 обещает «честный отказ» — поправить формулировку под Р1: по
  умолчанию общий ответ с пометкой, по желанию компании — строгий отказ.
- Согласие на загрузку документа «RAG-подсистема» в Алису AI для
  сравнения (задача ML 4).
- Формула приоритета пробелов (`domain.gaps.cluster_priority`).

---

# Приложение. Пункты BH-1…BH-26 — история (v1–v1.2)

Исходные пункты с контрактами, приёмкой и ловушками, как они были
поставлены; актуальный статус — в разделе 2. Блок «Статус у бэкенда»
внутри приложения датирован вечером 25.09.
...
| `ml/multi-query` | #18 | M6 multi-query: `prompts/multi_query.py`, `fuse_query_rankings`, стенд; по замеру не включаем | — (только стенд) |

#7–#18 вливаются после #6 — CI у них падает на той же ожидаемой ошибке.
...
#### BH-27. Запасная модель при `content_filter` (P3, решение продукта; 26.09)

BH-25 уже превращает `content_filter` в честный отказ. Но на золотом dev
фильтр Flash срабатывает не на 0,1 % вопросов, как на серебряных, а на
**1 из 26 вопросов по документам** — g15 «Можно за деньги гранта
Старт-ИИ-1 заказать разработку у дружественной компании, которая с нами
аффилирована?»: в 6 прогонах из 7 модель отвечает «Я не могу обсуждать
эту тему» (в гибриде — нет: другие выдержки, то есть срабатывание зависит
от контекста). Для грантовых документов такие вопросы — норма
(аффилированные лица, списки террористов и экстремистов как основание
для отказа), и сотрудник получает отказ там, где ответ в документах есть.

- **Предложение:** при `content_filter` — один запасной вызов другой
  модели с тем же промптом: `yandexgpt-lite` через нативный API
  (адаптер уже есть). Не второй Flash: при T = 0 он ответит так же.
  Ответ запасной модели — обычный ответ по документам (`origin=documents`),
  в `qa_log` — её `llm_model` и `llm_model_version`, чтобы видеть долю
  таких ответов.
- **Цена:** +1 вызов только в этих случаях (~0,4 ₽ у Lite); задержка
  +2–4 с.
- **Приёмка:** тест `finish_reason="content_filter"` у первого адаптера →
  второй адаптер вызван один раз, ответ 200 с источниками; у второго тоже
  `content_filter` → отказ, как в BH-25.
- **Ловушка:** не пытаться переформулировать вопрос, чтобы обойти
  фильтр, — это другой продукт и другая ответственность. Только другая
  модель со своей политикой.
- **Решение — за продуктом** (Артём): можно оставить BH-25 как есть и
  принять, что ~4 % вопросов по грантовым положениям получат отказ.
