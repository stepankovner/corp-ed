...
### Статус у бэкенда (ветка `claude/gallant-pascal-xtb35t`, 25.09)

- ML-стек влит до `ml/alice-compare` e6533d6. В 7f2aa4f закрыты BH-1,
  BH-3, BH-7, BH-8, BH-9, BH-10. Ожидаемые падения ниже там уже ушли.
- **Не сделано:** BH-6 и BH-15…BH-18. Сейчас там `text-search`,
  `Vector(256)`, `EMBEDDING_DIM = 256` в `llm/yandex_embedding.py`,
  `yandexgpt-lite`, порог 0.65.
...
- ML-ветки обновляются только новыми коммитами и слияниями, без
  переписывания истории. Чтобы забрать изменения, влейте `ml/alice-compare`
  ещё раз.
