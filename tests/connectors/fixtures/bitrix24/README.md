...
пути `/rest/{user}/{code}/`; адрес тестового портала в ссылках заменён на
`b24-test.bitrix24.ru`. Отсутствие значений переменных `BITRIX24_TEST_*`
(портал, вебхук, код и секрет приложения) в файлах проверено скриптом
перед коммитом. Записи базы знаний,
